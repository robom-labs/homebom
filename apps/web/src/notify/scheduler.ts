...
export function collectPendingAlerts(notices: Notice[], subs: SubMap, now: number): NoticeAlert[] {
...
export function collectDueAlerts(
  ...
  return collectPendingAlerts(notices, subs, now - graceMs).filter(
    (a) => a.fireAt <= now && !(a.id in fired),
  );
}

/** 15초 간격 + 화면 복귀 시점에 도래 알림을 확인해 울린다. 반환값은 정리 함수다. */
export function startAlertScheduler(getState: () => { notices: Notice[]; subs: SubMap }): () => void {
  const check = () => {
    const { notices, subs } = getState();
    for (const alert of collectDueAlerts(notices, subs, Date.now())) {
      markFired(alert.id);
