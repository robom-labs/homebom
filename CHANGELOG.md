...
### 변경
- 서비스워커 `install` 단계에서 앱 셸(`manifest.webmanifest`, 아이콘 3종)을 사전 캐시하도록 추가했다. 기존에는 첫 방문 시 fetch 이벤트를 통해서만 캐시가 채워져, 설치 직후 오프라인이면 아이콘·매니페스트가 비어 있을 수 있었다. 사전 캐시 경로는 `sw.js` 기준 상대경로(`./`)라 base path(`/zoopzoopcall/`)는 그대로다. 개별 자산 캐시 실패가 install 전체를 막지 않도록 각 요청을 개별 처리했다.
- 캐시 이름을 `zzc-v1` → `zzc-v2`로 올려 기존 사용자에게도 새 서비스워커가 새 캐시로 교체되게 했다.
